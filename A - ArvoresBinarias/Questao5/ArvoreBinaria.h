...
int MenuArvore()
{
    ...
    cout << "  [5] VERIFICAR SE A ARVORE EH CHEIA                    " << endl;
...
void PreOrdem(Arvore Tree)
{
    if (Tree != NULL)
    {
        cout << "Chave: " << Tree->chave << " - Info: " << Tree->info << endl;
        PreOrdem(Tree->esq);
        PreOrdem(Tree->dir);
    }
}
...
bool ArvoreCheia(Arvore Tree)
{
    ...
    if (Tree->dir == nullptr && Tree->esq == nullptr)
    {
        return true;
    }
    ...
    if (Tree->dir != nullptr && Tree->esq != nullptr)
    {
        return ArvoreCheia(Tree->esq) && ArvoreCheia(Tree->dir);
    }
