...
public:
	...
	//---------------------------------------------------------------------------
	//                             FUNCOES DE FLOYD-WARSHALL
	//---------------------------------------------------------------------------

	void FloydWarshall()
	{
		cout << "\n\tExecutando algoritmo de Floyd-Warshall..." << endl;

		int **distancia = (int **)malloc(V * sizeof(int *));
		for (int i = 0; i < V; i++)
		{
			distancia[i] = (int *)malloc(V * sizeof(int));
			for (int j = 0; j < V; j++)
			{
				if (i == j)
					distancia[i][j] = 0;
		...
		cout << "\n\t--------------------------------------------------" << endl;
		cout << "\t  Matriz de Menores Distancias (Floyd-Warshall)" << endl;
		cout << "\t--------------------------------------------------" << endl;
		cout << "\t   ";
		for (int j = 0; j < V; j++)
			cout << "\t[" << j << "]";
		...
		cout << "\n\tAlgoritmo concluido!" << endl;
		system("pause");
	}

	void FloydWarshallComCaminho()
	{
		cout << "\n\tExecutando Floyd-Warshall com reconstrucao de caminho..." << endl;


		int **distancia = (int **)malloc(V * sizeof(int *));

		int **proximo = (int **)malloc(V * sizeof(int *));

		for (int i = 0; i < V; i++)
		{
			distancia[i] = (int *)malloc(V * sizeof(int));
			proximo[i] = (int *)malloc(V * sizeof(int));
			for (int j = 0; j < V; j++)
			{
				if (i == j)
				{
		...
		cout << "\n\t--------------------------------------------------" << endl;
		cout << "\t           Matriz de Menores Distancias" << endl;
		cout << "\t--------------------------------------------------" << endl;
		cout << "\t   ";
		for (int j = 0; j < V; j++)
			cout << "\t[" << j << "]";
		...
		cout << "\n\tExemplos de caminhos mais curtos:" << endl;
		cout << "\t--------------------------------------------------" << endl;
		...
		for (int i = 0; i < V; i++)
		{
			for (int j = i + 1; j < V; j++)
			{
				if (distancia[i][j] != inf)
				{
					cout << "\tDe " << i << " para " << j << " (dist=" << distancia[i][j] << "): ";

					int atual = i;
					cout << atual;
					while (atual != j)
					{
						atual = proximo[atual][j];
						if (atual == -1)
							break;
						cout << " -> " << atual;
					}
